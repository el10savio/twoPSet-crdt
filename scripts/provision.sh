...
# of the twopset peers
declare -a peers=()
declare -a peer_id_list=()

# Name of the TwoPSet cluster network
# connecting the peers
network="twopset_network"
...
echo "Provisioning TwoPSet Docker Cluster"

echo "Building TwoPSet Docker Image"
docker build -t twopset -f Dockerfile .

if [[ $? -ne 0 ]]; then
    echo "Unable To Build TwoPSet Docker Image"
    exit 255
fi

echo "Building TwoPSet Cluster Network"
...
for peer_index in "${!peers[@]}"; do
    docker run -p "${peers[$peer_index]}":8080 --net $network -e "PEERS="$comma_separated_peer_id_list"" -e "NETWORK="$network"" --name="peer-$peer_index" -d twopset
done
